...
export interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'primary' | 'secondary' | 'ghost' | 'danger' | 'illuminated' | 'outline' | 'default' | 'destructive' | 'link';
...
// Export buttonVariants for Radix UI components that need it
// This is a simplified version for compatibility
export const buttonVariants = (props?: { variant?: string; size?: string; className?: string }) => {
  const variant = props?.variant || 'secondary';
  const size = props?.size || 'default';
  const baseStyles = 'inline-flex items-center justify-center gap-2 font-medium focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 disabled:opacity-40 disabled:cursor-not-allowed disabled:pointer-events-none border select-none transition-[background-color,border-color,color,box-shadow,transform] duration-100 ease-out';
  
  const variantStyles: Record<string, string> = {
    default: 'bg-[var(--color-bg-surface-elevated)] text-[var(--color-text-primary)] border-[var(--color-border-strong)]',
    outline: 'border-[var(--color-border-strong)] bg-transparent hover:bg-[var(--color-overlay-hover)]',
    secondary: 'bg-[var(--color-bg-surface-elevated)] text-[var(--color-text-primary)] border-[var(--color-border-strong)]',
  };
  ...
  return `${baseStyles} ${variantStyles[variant] || variantStyles.default} ${sizeStyles[size] || sizeStyles.default} ${props?.className || ''}`;
};
