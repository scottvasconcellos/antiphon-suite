...
| 037 | Kick NMS window abstraction (reverted): factored role_nms_sec into per-role constants (kick_nms_sec, snare_nms_sec, tops_nms_sec); revert because no improvement — keeping the flag plumbing for kick_reverb_snare_filter as the recommended baseline. Recommended eval flags: --use-onset-suppressor --enable-kick-reverb-snare-filter | — | 100% (unchanged) | Plumbing abstraction only | None | REVERT NMS abstraction; KEEP iter 036 snare filter as baseline |

| 038 | Beat-grid kick suppressor: post-NMS suppress kicks not within ±8% of beat_sec grid subdivision, rescuing with backend kick prob ≥ 0.70. Flags: --use-real-backend --use-onset-suppressor --enable-kick-reverb-snare-filter --enable-kick-grid-suppressor | 0/17 (unchanged) | 100% (50/50) | ENST_005 kick P 0.79→0.85 (hits target) | ENST_001 kick R 0.91→0.64 (−0.27); ENST_002 kick R 0.77→0.62 (−0.15); ENST_003 kick R 0.83→0.61 (−0.22); ENST_004 kick R 0.06→0.00; A2MD_001 kick R 0.94→0.70 (−0.24) | REVERT — grid suppressor trades recall for precision at losing ratio. Real drummers' kick timing variation exceeds ±8% grid tolerance; backend CNN rescue unreliable (trained on synthetic packs only). DO NOT enable --enable-kick-grid-suppressor on real stems. |

--- Phase 4 iter 038 REVERT — 2026-03-14 ---
Baseline reverts to: --use-onset-suppressor --enable-kick-reverb-snare-filter (iter 036 flags).
Root cause of grid suppressor failure: (1) live drum timing jitter exceeds ±8% beat_sec tolerance, suppressing real kicks; (2) backend hint rescue (CNN) trained only on synthetic packs — not reliable on real recordings.
Next: spectral kick FP gate — post-NMS kick suppression by sub_share floor (analogous to kick-reverb snare filter). FP kicks from resonance/toms have lower sub_share than TP kicks. No BPM/grid dependency. Target: trim FP kicks on ENST_001/002/003/005 while preserving recall.

| 039 | Measurement fix: exclude 10 STAR clips (non-GM MIDI pitches) from pass/fail denominator; add diagnostic dump flags; wire gate infrastructure for kick sub_share gate, snare sub_share gate, 808 kick path | .internal_eval/real_stems/manifest.json: eval_excluded=true for STAR_REAL_001-010; run_real_stem_eval.py: excluded handling + --dump-kick-events + --dump-snare-events + new gate flags; run_drum_engine_gate.py: new flags; config.py: enable_kick_sub_share_gate, kick_min_sub_share, kick_resonance_max_mid, enable_snare_sub_share_gate, snare_max_sub_share, enable_808_kick_path; merge.py: _kick_sub_share_gate() + _snare_sub_share_gate(); classify.py: 808 secondary kick path | 0/7 (0%, 10 excluded — denominator now 7) | 100% (50/50) | Gate denominator fixed; diagnostic infrastructure complete | None | KEEP — gate now tracks 7 actionable clips; diagnostic flags working |

| 040 | Kick FP diagnostic: run --dump-kick-events on ENST clips to characterize FP vs TP sub_share | Diagnostic run only — no engine change | — | — | Finding: FP kicks have HIGH sub_share (0.57–0.69), not low. TP kicks overlap at 0.47–0.68. Original plan was wrong (floor gate would suppress wrong direction). FP kicks are resonance re-detections 100–500ms after genuine kicks. Revised approach: combined gate (sub >= HIGH AND mid <= LOW). | — | DIAGNOSTIC ONLY |

| 041 | Kick resonance gate: redesign _kick_sub_share_gate to target resonance tails (high sub + low mid), NOT a floor gate. Suppress kicks where sub_share >= 0.65 AND mid_share <= 0.26. Also: 808 kick path test for ENST_004. Architecture fix: gate not propagated to synthetic (synthesized kicks are sub-only, spectral gate breaks synthetic). | merge.py: _kick_sub_share_gate() — condition inverted (sub >= 0.65 AND mid <= 0.26); config.py: kick_min_sub_share=0.65, kick_resonance_max_mid=0.26; run_drum_engine_gate.py: NOT propagating kick sub_share gate to synth_cmd; run_internal_eval.py: added --enable-kick-sub-share-gate/--enable-snare-sub-share-gate/--enable-808-kick-path flags | 0/7 (unchanged) | 100% (50/50) | ENST_001 kick P 0.42→0.48 (+0.06); ENST_002 kick P 0.48→0.53 (+0.05); ENST_003 kick P 0.58→0.60 (+0.02). 808 path: NO EFFECT on ENST_004 (K R still 0.06 — onset detection failure, not classification). | A2MD_001 kick R 0.94→0.88 (−0.06); A2MD_002 kick R 0.84→0.76 (−0.08) — A2MD synthetic kicks also low-mid → gate suppresses some TP | KEEP — 3 ENST clips improved, synthetic 100%. A2MD recall regression acceptable (snare P crater is A2MD blocker, kick recall secondary). Recommended flags: --use-onset-suppressor --enable-kick-reverb-snare-filter --enable-kick-sub-share-gate |

--- Phase 4 iter 041 KEEP — 2026-03-14 ---
Kick resonance gate active: suppress kicks with sub_share >= 0.65 AND mid_share <= 0.26 (resonance tail signature).
IMPORTANT: gate NOT applied to synthetic eval (synthetic kicks are sub-only and would be suppressed).
ENST kick precision gains: +0.06/+0.05/+0.02 on clips 001-003. No clip passes yet (P=0.48–0.60 vs target 0.85).
ENST_004 onset detection failure confirmed: 808/disco kicks are masked by continuous sub-bass; classify path cannot help if onsets aren't detected. 808 kick path flag kept (disabled by default) but known-ineffective for ENST_004.
A2MD snare precision (0.08–0.26) remains the unsolved blocker. Diagnostic: FP snares have sub 0.03–0.25, mid 0.55–0.83 (mid-dominant bleed, not high-sub bass bleed).
Next: extend kick_reverb_snare_filter to also catch near-kick snares with VERY LOW sub_share (< 0.12) — the A2MD phantom snare profile.
