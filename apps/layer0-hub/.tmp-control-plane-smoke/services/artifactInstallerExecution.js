...
function serializeTreeHash(entries) {
    return sha256(entries.map(([path, content]) => `${path}:${sha256(content)}`).join("|"));
}
function toManifestVersion(manifest) {
    return `${manifest.appVersion}:${manifest.channel}`;
}
function collectTargetEntries(fileSystem, targetDir) {
...
export function applyArtifactManifest(input) {
    ...
    const nextFileSystem = { ...input.fileSystem };
...
export function executeAtomicArtifactTransaction(input) {
