import { parseArtifactManifest } from "./artifactManifestContract.js";
import { ARTIFACT_SIGNATURE_MAX_SKEW_SECONDS, toEpochSeconds } from "./timeControl.js";
...
function computeDeterministicSignatureHash(payload, keyId) {
    let hash = 0;
...
export function verifyArtifactTrust(input) {
    const parsed = parseArtifactManifest(input.manifestRaw);
    if (!parsed.manifest) {
        return {
            trusted: false,
            reasonCode: parsed.reasonCode === "invalid_artifact_manifest_json"
            ...
            remediation: parsed.reasonCode === "invalid_artifact_manifest_json" || parsed.reasonCode === "invalid_artifact_manifest_shape"
...
export function buildDeterministicSignature(manifestRaw, keyId) {
