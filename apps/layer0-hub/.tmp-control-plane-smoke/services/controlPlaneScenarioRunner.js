...
import { runInstallUpdateAuthority, runInstallUpdateAuthorityWithArtifactExecutor } from "./installUpdateAuthority";
...
export async function runHappyPathScenario(seed) {
...
export async function runFailureScenario(seed, action, reasonCode) {
    const target = seed.entitlements[0];
    if (!target) {
    ...
    const recovery = transitionLifecycleState(failed.result.lifecycle.to, "AcknowledgeFailure");
    return {
...
export function runAntiGatingScenario(seed) {
...
export function runHubOptionalScenario(seed) {
    ...
    const serializedTrust = serializePersistedControlPlaneState(toPersistedControlPlaneState(seed, {
        input: {
            identity: { authenticated: seed.session !== null },
            license: { owned: seed.entitlements.some((app) => app.owned), revoked: false },
            offlineCache: {
...
export async function runLongRunDeterminismScenario(seed, cycles) {
    let snapshot = seed;
    ...
    for (let i = 0; i < cycles; i += 1) {
        ...
        const persisted = toPersistedControlPlaneState(update.snapshot, {
            input: {
...
export function runTrustEnvelopeValidationScenario(seed) {
    const entitlement = decideEntitlement({
        identity: { authenticated: seed.session !== null },
        license: { owned: seed.entitlements.some((app) => app.owned), revoked: false },
        offlineCache: {
...
export async function runConcurrencyScenario(seed) {
    const target = seed.entitlements[0];
    if (!target) {
...
export function runTrustArtifactScenario(seed) {
...
export async function runArtifactInstallUpdateScenario(input) {
    ...
    const updateSeed = {
        ...install.snapshot,
        entitlements: install.snapshot.entitlements.map((app) => app.id === input.appId ? { ...app, updateAvailable: true } : app)
    };
    ...
    const installFailure = await runInstallUpdateAuthorityWithArtifactExecutor(input.seed, "install", input.appId, {
        manifestRaw: input.installManifestRaw,
        payloadFiles: input.payloadFiles,
        targetDir: input.targetDir,
        fileSystem: {},
    ...
    const failingUpdate = await runInstallUpdateAuthorityWithArtifactExecutor(updateSeed, "update", input.appId, {
        manifestRaw: input.updateManifestRaw,
        payloadFiles: input.payloadFiles,
        targetDir: input.targetDir,
        fileSystem: install.fileSystem,
...
export async function runRealLayerAppPipelineScenario(input) {
    ...
    const updateSeed = {
        ...install.snapshot,
        entitlements: install.snapshot.entitlements.map((app) => app.id === input.appId ? { ...app, version: "1.1.0", updateAvailable: true } : app)
    };
    ...
    const hubOptional = runTrustEnvelopeValidationScenario({
        ...update.snapshot,
        entitlements: update.snapshot.entitlements.map((entry) => entry.id === input.appId ? { ...entry, owned: true, installedVersion: "1.1.0" } : entry)
