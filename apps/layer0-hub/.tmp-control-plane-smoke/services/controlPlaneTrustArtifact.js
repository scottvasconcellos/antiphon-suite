...
export function issueTrustArtifact(snapshot) {
    return {
...
export function serializeTrustArtifact(artifact) {
    return JSON.stringify(artifact);
}
export function parseTrustArtifact(raw) {
    return parseTrustArtifactWithReport(raw).artifact;
}
export function parseTrustArtifactWithReport(raw, options) {
    let parsed;
    try {
    ...
    if (!Array.isArray(candidate.appIds) ||
...
export function normalizeTrustArtifact(artifact) {
    return {
        schema: "antiphon.trust-artifact",
        version: 1,
...
export function isRunnableWithoutHub(artifact, appId) {
    if (!artifact) {
        return false;
    }
