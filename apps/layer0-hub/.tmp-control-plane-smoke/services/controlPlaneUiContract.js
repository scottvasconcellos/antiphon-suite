export function toControlPlaneUiContract(hubVm, controlPlaneVm, opsVm) {
    const entitlementLabel = controlPlaneVm.entitlement.outcome === "Authorized"
        ? "Allowed"
        : controlPlaneVm.entitlement.outcome === "OfflineAuthorized"
            ? "OfflineAllowed"
            : "Denied";
    const installLabel = controlPlaneVm.installUpdate.reasonCode.startsWith("ok_")
        ? "Installed"
        : controlPlaneVm.installUpdate.reasonCode.includes("update")
            ? "Updating"
            : "Failed";
    const launchLabel = (ready) => (ready ? "Runnable" : "NotRunnable");
    ...
    return {
        ...
        entitlementLine: appEntries.length === 0
